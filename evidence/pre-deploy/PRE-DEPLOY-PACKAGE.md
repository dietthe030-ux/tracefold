...
- Direct Mode via the isolated current toolchain: 25/25 PASS. It covers construction/bounds, grammar, replay protection, every public objection reason, all outcomes, cluster extension, cross-cluster conflict with no mutation, unavailable/malformed/oversized source data, prompt injection, a forged model-only `SAME` claim over unrelated ecosystems, exact-reference boundary rejection, validator disagreement, retry/cooldown, final fingerprint reproduction, upgrade authorization, pagination, exact-once consumption/history, and objection mutation guard. Output: `contract-tests.txt`.
- Live read-only `getContractSchemaForCode` against the exact candidate: PASS. Constructor and all 20 intended public methods are present in `live-schema.json`, including bounded `get_paged_consumptions`.
- Frontend: 77/77 PASS, TypeScript PASS, production build PASS. The added regressions reject pre-existing-objection and unchanged-UNRESOLVED readbacks, prove exact positive deltas, fail closed when finality is absent, restrict the primary field to CVE, and verify the consumption-history ABI. Outputs: `frontend-tests.txt`, `frontend-typecheck.txt`, `frontend-build.txt`.
...
Frontend callers match the live ABI, including `get_assessment_history(proposal_id, offset, limit)` and `get_paged_consumptions(offset, limit)`. Pagination is capped at 20 and assessment history at 3. Objection choices use exactly the five contract codes, and the primary proposal input accepts the required CVE family only.
...
The production bundle keeps a 613.66 kB minified GenLayer SDK chunk (132.50 kB gzip). Application code is separately 104.88 kB (25.68 kB gzip). This is a dependency-size warning, not a functional failure; replacing or partially vendoring the current SDK would weaken exact-version integrity.

## Trust and transaction gates

Public source bodies and wallet metadata are untrusted. Contract authorization derives only from caller/address state. Browser writes bind to the selected callable provider/account, verify chain and spendable balance, estimate exact fees, persist the intent and method-specific pre-state before signing, retain the returned hash, require `FINALIZED` plus semantic success, then verify the exact authoritative state delta. Unknown post-hash results remain `RECONCILIATION_REQUIRED`; no automatic resubmission exists.
